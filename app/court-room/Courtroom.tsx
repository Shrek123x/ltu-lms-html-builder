...
/**
 * CourtRoom - Stage 3 (Complete Requirements)
 *
...
// TIMING CONSTANTS - Configurable intervals for demo/testing
// NOTE: These are SHORT for demo purposes (20-30s). In production, use longer intervals.
const DEMO_REMIND_MIN_MS = 20_000; // first interval pick: 20s (for quick demo)
const DEMO_REMIND_MAX_MS = 30_000; // 30s
const ESCALATE_WAIT_MS = 120_000; // 2 minutes for escalation -> urgent or court scheduling
...
export default function CourtRoom() {
  ...
  /* Code debugging interface (NEW: allows user to fix code to resolve messages) */
  ...
  /* ------------------------
     Timer logic (unchanged from earlier)
     ------------------------ */
  ...
  /* ========================
     ...
     Can be commented out or removed for production deployment.
     ======================== */

  // TEST FUNCTION: Force immediate escalation (for quick testing of escalation flow)
  // PURPOSE: Allows developers to test urgent/court scenarios without waiting 2+ minutes
  // PRODUCTION: Comment out or remove this function and its button in the UI
  function forceEscalate(id: string) {
    setMessages((prev) =>
      prev.map((m) => {
        if (m.id !== id) return m;
        const newLevel = escalateLevel(m.level);
        const escalations = (m.escalations ?? 0) + 1;
        // If moving to urgent, schedule courtAt in 2 minutes
        const courtAt = newLevel === "urgent" ? Date.now() + ESCALATE_WAIT_MS : undefined;
        return { ...m, level: newLevel, escalations, nextRemindAt: courtAt ? undefined : Date.now() + ESCALATE_WAIT_MS, courtAt };
      }),
    );
  }
  ...
  /* ------------------------
     ...
     ------------------------ */
  // TEST FUNCTION: Add sample messages for quick testing
  // PURPOSE: Quickly populate the UI with test data without waiting for scheduler
  // PRODUCTION: Can be removed or kept for demo purposes
  function addSampleMessages() {
    const stage = STAGES[stageIndex];
    for (const pick of stage.messages.slice(0, 3)) {
      addMessage({
        id: genId("sample"),
        text: pick.text,
        from: pick.from,
  ...
  /* ------------------------
     Code debugging functions (NEW REQUIREMENT: user must debug code to resolve issues)
      ...
      */}
      ...
      <div style={{ position: "relative", zIndex: 2, color: "white", padding: 16 }}>
        {/* 
          ...
          - Quick action buttons for adding test messages
        */}
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            {/* These buttons are for development/testing only - comment out or remove before deployment */}
            
            {/* TEST BUTTON: Manually add a specific message */}
            <button onClick={() => addMessage({
                id: genId("manual"),
                text: "Manual: Fix alt in img1",
                from: "agile",
                level: "info",
                consequence: "accessibility",
              })} disabled={appDisabled} style={{ padding: "8px 10px", borderRadius: 6 }}>
              Add "alt" message
            </button>

            {/* TEST BUTTON: Add sample messages from current stage */}
            <button onClick={addSampleMessages} disabled={appDisabled} style={{ padding: "8px 10px", borderRadius: 6 }}>
              Add samples
            </button>
            {/* ========== END TEST BUTTONS ========== */}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 12, marginTop: 12 }}>
          {/* 
            ...
            - Action buttons: Debug Code (if challenge exists), Resolve, Force escalate
          */}
          <div>
            <section style={{ background: "rgba(255,255,255,0.04)", padding: 12, borderRadius: 8 }}>
              ...
              <div style={{ maxHeight: 420, overflow: "auto" }}>
                {messages.length === 0 ? (
                  <div style={{ opacity: 0.8 }}>No messages yet — wait for scheduled ones or add samples.</div>
                ) : (
                          ...
                          {!m.resolved && !appDisabled ? (
                            <>
                              ...
                              */}
                              ...
                              {/* TEST BUTTON: Force escalation for quick testing - REMOVE FOR PRODUCTION */}
                              <button onClick={() => forceEscalate(m.id)} style={{ padding: "6px 8px", borderRadius: 6 }}>
                                Force escalate
                              </button>
                            </>
                          ) : (
          ...
          */}
          <aside style={{ background: "rgba(0,0,0,0.5)", padding: 12, borderRadius: 8, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            */}
            ...
            {/* 
              CODE DEBUGGING INTERFACE (NEW FEATURE)
